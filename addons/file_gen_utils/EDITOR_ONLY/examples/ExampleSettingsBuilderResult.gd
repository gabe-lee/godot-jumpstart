...
const ExampleSettingsBuilderResult = preload("res://addons/file_gen_utils/EDITOR_ONLY/examples/ExampleSettingsBuilderResult.gd")
...
const FMT_SIG: PackedByteArray = [83, 67, 65, 86, 83, 69, 84, 84]
...
signal file_saved(file: ExampleSettingsBuilderResult)
signal file_loaded(file: ExampleSettingsBuilderResult)
...
func _load_to_dict_then_upgrade(file: ConfigFile) -> void:
    ...
    var read_routine: Callable = ExampleSettingsBuilderResult.PREV_VERSION_READ_ROUTINES[format_version]
    var vals: Dictionary = {}
    read_routine.call(file, vals)
    while format_version < MAX_VERSION:
        var upgrade_routine: Callable = ExampleSettingsBuilderResult.PREV_VERSION_UPGRADE_ROUTINES[format_version]
...
func clone_metadata_only(disable_autosave_on_clone: bool = false) -> ExampleSettingsBuilderResult:
    var new_self := ExampleSettingsBuilderResult.new(self.file_path)
...
func data_equals(other: ExampleSettingsBuilderResult, _is_integrity_check: bool = false) -> bool:
